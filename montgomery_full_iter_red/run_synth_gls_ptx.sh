...
#for ZBITS in $(seq $((($NBITS/16))) $((NBITS/16)) $(($NBITS-1))); do     
     if [[ $SYNTH_ONLY == 1 ]]; then
         (echo "Running Synthsis." && cd synth && ./run_synth.sh -design $DESIGN -nbits  $NBITS -zbits  $(($ZBITS-1))    -pbits $PBITS -clkname $CLKNAME  -clkperiod  ${CLKPERIOD[$CLK_IDX]}  -tech $TECHNODE -lib $TECHLIB -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && echo "Synthesis Completed." && cd ../ ) &
     elif [[ $POWER_ONLY == 1 ]]; then
         (echo "Running GLS." && cd gls && export SYNOPSYS_SIM_SETUP=/data/nyu_projects/dss545/barrett_red_DAC/gls/synopsys_sim.setup &&./run_verif.sh -design $DESIGN -tb $TESTBENCH -ntrace $NTRACES -nbits $NBITS -zbits $(($ZBITS-1))  -pbits $PBITS -tech $TECHNODE -clkperiod ${CLKPERIOD[$CLK_IDX]} -rperiod ${RPERIOD[$CLK_IDX]} -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "GLS Completed." && echo "Running PTPX." && cd ptpx &&./run_ptpx.sh -design $DESIGN -tb $TESTBENCH -nbits $NBITS -zbits $(($ZBITS-1))   -pbits $PBITS -clkname $CLKNAME -clkperiod ${CLKPERIOD[$CLK_IDX]} -lib $TECHLIB_PTPX -tech $TECHNODE -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "PTPX Completed." && echo "Verification Started" && cd verif && ./run_verif.sh -design $DESIGN -tb $TESTBENCH -ntrace $NTRACES -nbits $NBITS -zbits $(($ZBITS-1))  -pbits $PBITS -tech $TECHNODE -clkperiod ${CLKPERIOD[$CLK_IDX]} && cd ../ && echo "Verification Completed.") &
     elif [[ $VERIF_ONLY == 1 ]]; then
         (echo "Verification Started" && cd verif && ./run_verif.sh -design $DESIGN -tb $TESTBENCH -ntrace $NTRACES -nbits $NBITS -zbits $(($ZBITS-1))  -pbits $PBITS -tech $TECHNODE -clkperiod ${CLKPERIOD[$CLK_IDX]} -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "Verification Completed.") &
     else
         (cd synth && ./run_synth.sh -design $DESIGN -nbits  $NBITS -zbits  $(($ZBITS-1))    -pbits $PBITS -clkname $CLKNAME  -clkperiod  ${CLKPERIOD[$CLK_IDX]}  -tech $TECHNODE -lib $TECHLIB -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && echo "Synthesis Completed." && cd ../ && echo "Running GLS." && cd gls && export SYNOPSYS_SIM_SETUP=/data/nyu_projects/dss545/barrett_red_DAC/gls/synopsys_sim.setup &&./run_verif.sh -design $DESIGN -tb $TESTBENCH -ntrace $NTRACES -nbits $NBITS -zbits $(($ZBITS-1))  -pbits $PBITS -tech $TECHNODE -clkperiod ${CLKPERIOD[$CLK_IDX]} -rperiod ${RPERIOD[$CLK_IDX]} -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "GLS Completed." && echo "Running PTPX." && cd ptpx &&./run_ptpx.sh -design $DESIGN -tb $TESTBENCH -nbits $NBITS -zbits $(($ZBITS-1))   -pbits $PBITS -clkname $CLKNAME -clkperiod ${CLKPERIOD[$CLK_IDX]}   -lib $TECHLIB_PTPX -tech $TECHNODE -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "PTPX Completed." && echo "Verification Started" && cd verif && ./run_verif.sh -design $DESIGN -tb $TESTBENCH -ntrace $NTRACES -nbits $NBITS -zbits $(($ZBITS-1))  -pbits $PBITS -tech $TECHNODE -clkperiod ${CLKPERIOD[$CLK_IDX]} -mstage1 ${MSTAGE1[$CLK_IDX]} -mstage2 ${MSTAGE2[$CLK_IDX]} -mstage3 ${MSTAGE3[$CLK_IDX]} && cd ../ && echo "Verification Completed.") &
     fi
done
#wait
#CLK_IDX=$(($CLK_IDX+1))
